export const codeMap = codeMapGenerator();
export const reservedCodepoints = 7151;

function codeMapGenerator() {
	function set_codepoint(codepoint: number) {
		unicode_set.push(codepoint);
	}

	function set_codepoints(start: number, stop: number) {
		for (let i = start; i <= stop; i++) {
			unicode_set.push(i);
		}
	}

	function unset_codepoint(codepoint: number) {
		unicode_set = unicode_set.filter((cp) => cp != codepoint);
	}

	function unset_codepoints(start: number, stop: number) {
		for (let i = start; i <= stop; i++) {
			unicode_set = unicode_set.filter((cp) => cp != i);
		}
	}

	let unicode_set: number[] = [];
