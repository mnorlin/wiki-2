...
export function lcg(seed: bigint, { a, c, m }: LcgParams) {
	return (a * seed + c) % m;
}

// Euclidean algorithm, finds greatest common divisor
export function gcd(a: bigint, b: bigint): bigint {
	return a != 0n ? gcd(b % a, a) : b;
}

// Find the seed of an LCG generated number
export function reverseLcg(number: bigint, { a, c, m }: LcgParams) {
...
export function verifyLcgParams({ a, c, m }: LcgParams) {
	function divisibleAllPrimes(a: bigint, b: bigint): boolean {
		if (b === 1n) return true;
		const c = gcd(a, b);
